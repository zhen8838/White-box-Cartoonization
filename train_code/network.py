...
if __name__ == '__main__':
    ...
    # input_photo = tf.placeholder(tf.float32, [1, 256, 256, 3])
    ...
    real = tf.placeholder(tf.float32, [1, 256, 256, 3])
    scale = 1
    patch = True
    name = 'disc_gray'
    channel = 32
    output = disc_sn(real, scale, channel, name=name, patch=patch, reuse=False)
    print(output.shape)
    slim.model_analyzer.analyze_vars([var for var in tf.trainable_variables() if var.name.startswith(name)], print_info=True)
