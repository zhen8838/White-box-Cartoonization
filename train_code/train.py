...
def train(args):
    ...
    train_writer = tf.summary.FileWriter(args.save_dir + '/train_log', sess.graph)
    ...
    with tf.device('/device:GPU:0'):
        ...
        for total_iter in tqdm(range(1000, 1000 + args.total_iter)):
            ...
            """ NOTE 尝试一下不使用superpixel_batch看看能不能成功运行
                可以正常运行，因为训练判别器的确不需要用到superpixel_batch
            """
            # _, d_loss = sess.run([d_optim, d_loss_total],
            #                      feed_dict={input_photo: photo_batch,
            #                                 # input_superpixel: superpixel_batch,
            #                                 input_cartoon: cartoon_batch})
            # train_writer.add_summary(train_info, total_iter)
